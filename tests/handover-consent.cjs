...
assert(js.includes('data-handover-row'));
assert(js.includes("detailRow.className='inline-detail-row'"));
assert(js.includes("동의자 ${item.consentedName || '인수자'}"));
assert(!js.includes('maskedMemberId'));
assert(!html.includes('id="handoverDetail"'));
