...
4. 배포 대상은 `dist/` 전체다. index.html만 업로드하면 CSS와 기능 파일이 빠진다.

별도 빌드나 npm 설치 없이 정적 파일로 동작한다. 외부 SheetJS 로딩은 기존과 동일하다.
실행 순서: HTML 구성 → SheetJS → dialogs.js에서 팝업 구성 → app.js에서 DOM 연결과 초기 화면 표시.
dialogs.js와 app.js에 임의로 async를 추가하지 않는다. 팝업 DOM 생성 전에 app.js가 실행되면 연결 오류가 발생할 수 있다.
