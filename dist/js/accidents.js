...
(() => {
  ...
  function openDetail(item,row) {
    ...
    if(!row)return;document.querySelectorAll('.inline-detail-row').forEach(item=>item.remove());detail.hidden=false;
  ...
  rowsEl.addEventListener('click',event=>{const edit=event.target.closest('[data-accident-edit]'),remove=event.target.closest('[data-accident-delete]'),button=event.target.closest('[data-accident-detail]');if(edit)openAccidentEdit(records[Number(edit.dataset.accidentEdit)]);else if(remove)openAccidentDelete(records[Number(remove.dataset.accidentDelete)]);else if(button)openDetail(records[Number(button.dataset.accidentDetail)],button.closest('tr'));});
  el('accidentDetail').addEventListener('click',event=>{if(event.target.closest('[data-accident-detail-close]')){el('accidentDetail').hidden=true;el('accidentDetail').closest('.inline-detail-row')?.remove();}});
