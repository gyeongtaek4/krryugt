...
(() => {
  ...
  function validatePhotos(photos) {
    if(photos.length>6)throw Error('사고 사진은 최대 6개까지 첨부할 수 있습니다.');
    let total=0;
    photos.forEach(photo=>{
      if(!['image/jpeg','image/png','image/webp'].includes(photo.type)||photo.size<1||photo.size>5*1024*1024)throw Error('JPG/PNG/WebP 사진만 파일당 5MB 이하로 첨부하세요.');
      total+=photo.size;
    });
    if(total>20*1024*1024)throw Error('사고 사진은 총 20MB 이하로 첨부하세요.');
  }
  ...
  function previewPhotos() {
    el('accidentPreview').innerHTML=photoDraft.map(photo=>`<figure><img src="${safe(photo.data)}" alt="${safe(photo.name)}"><figcaption>${safe(photo.name)}</figcaption></figure>`).join('');
  }
  function fileExtension(type) { return type==='image/png'?'png':type==='image/webp'?'webp':'jpg'; }
  async function signedPhotos(paths) {
    ...
    if(error)throw Error('사고 사진을 불러오지 못했습니다.');
  ...
  async function storeAccident(record) {
    ...
    try {
      for(let index=0;index<photoDraft.length;index++){
        ...
        if(error)throw Error(`사고 사진 저장에 실패했습니다: ${error.message||'Storage 오류'}`);
